...
\abstract{
    ...
    toe-ablation configuration. On the T-test, average and maximum path deviation decreased by 25.0\% and
    34.0\%, respectively, at a 4.4\% lower average speed and an essentially unchanged completion time,
    indicating improved path-tracking accuracy under a speed--accuracy trade-off.
}
...
Completion time is therefore essentially identical (a $0.1\%$ difference), and the two configurations differ
mainly in how that time is spent: the toe-equipped model tracks the prescribed path more closely, whereas the
toe-ablation model travels faster but departs further from the turns. The reduced deviation may stem from
improved turning dynamics, but a more conservative and slower policy cannot be ruled out. Fig.~\ref{fig:ttest}(b) and (c) show
the trajectories from the T-test trials. Compared with the toe-equipped case, the toe-ablation trajectories
depart from the reference path more often. The toe-equipped configuration therefore showed reduced path
deviation under the evaluated directional-change task.
...
at a 4.4\% lower average speed and an essentially unchanged completion time. Under the evaluated
directional-change task this indicates improved path-tracking accuracy rather than a general improvement in
agility.
