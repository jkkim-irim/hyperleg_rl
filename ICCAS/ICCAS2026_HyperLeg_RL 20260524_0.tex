...
\author{Joong-Gil Kim${}^{1}$, Wontae Ye${}^{1}$, Geunwoo Cho${}^{1}$, Seong-Ho Yun${}^{2}$, Se-Hyoung Cho${}^{3}$ and Yong-Jae Kim${}^{1,3,*}$}

\affils{${}^{1}$School of Electrical, Electronics and Communication Engineering, Korea University of Technology and Education, \\
Cheonan, 31253, Korea (giribboy97@koreatech.ac.kr; solnox99@koreatech.ac.kr; jgw58@koreatech.ac.kr; \\
yongjae@koreatech.ac.kr) {\small${}^{*}$ Corresponding author} \\
${}^{2}$Artificial Intelligence and Robotics Institute, Korea Institute of Science and Technology, \\
Seoul, 02792, South Korea (dnstjdgh@gmail.com) \\
${}^{3}$Robot Innovation Hub, WIRobotics Inc., \\
Cheonan, 31253, Korea (shcho@wirobotics.com)}
...
\abstract{
    ...
    replicated human characteristics nor rigorously validated their benefits. We propose a 14-DOF bipedal robot emulating
    human toes' lightweight, high-torque, robust nature. To quantitatively analyze the effectiveness of the active
    toes in terms of agility, efficiency, and impact absorption, we developed an actuator- and
    transmission-aware simulation training environment that reflects actual actuators with coupled
    transmissions and motor-side power consumption.
    To ensure a fair comparison between configurations with and without active toes, we applied an
    identical training procedure to both. The
    simulation results indicate that, at 1.33~m/s walking, the toe-equipped model reduced the cost of
    transport (CoT) by 17.5\% and heel-strike ground reaction force (GRF) by 5.0\% compared with the
    toe-ablation configuration. On the agility test, the toe-equipped model was 4.4\% slower on average
    but reduced mean path deviation by 25.0\%, completing the course in the same time to within 0.1\%.
}

\keywords{
    Bipedal robot, Active toe, Coupled actuation, Simulation, Reinforcement learning
}
...
Human locomotion is an intricate and efficient process, with the foot playing a crucial role in agility, efficiency, and impact absorption [1][2]. The human foot weighs approximately 1 kg and is composed of a 2 degree-of-freedom (DOF) ankle and multiple DOF toes [1]. Despite its relatively light weight, the foot can deliver up to 104 Nm of isometric torque and withstand ground reaction forces (GRF) equivalent to 2.5 times body weight during running [1]-[3]. The human toe, in particular, contributes significantly to
...
durability. For instance, Cassie has a single-DOF ankle [10], and Digit has a two-DOF ankle [12]; neither
incorporates an active toe joint, leaving the benefits that toes provide untapped.
...
spring-loaded toe mechanisms have been explored [18][19]. However, these approaches lack the dynamic
force modulation seen in human toes.
...
\begin{minipage}[b]{0.45\columnwidth}
    ...
    \end{subfigure}
\end{minipage}
\caption{(a) 14-DOF bipedal robot design.
...
consumes 289 W of metabolic power [1], corresponding to a CoT of 0.316. Traditional bipedal robots, in contrast, exhibit
...
\begin{figure}[t]
    ...
    \caption{(a) Dimensions of the 14-DOF bipedal robot.
...
a knee with one DOF, an ankle with two DOF, and a toe with one DOF. Since all heavy actuators
...
shown in Fig. 2(e) in parallel, enabling differential actuation for ankle roll and pitch. The leg
mechanism, including the active toe, is inherited unchanged from HyperLeg; see [20] for further details.
...
components are designed for low friction, high
backdrivability, and high impact resistance while maintaining
high power output. These features are intended to support
future sim-to-real transfer. The detailed specifications of
...
to ankle pitch. The last column shows that the toe motor likewise contributes to the knee and
ankle-pitch torques ($-31.11$ and $19.23$), so toe push-off is mechanically shared with the
proximal joints. This intentional coupling is termed Cooperative Actuation (CA) [23].
...
in locomotion research, inaccurate robot dynamics and actuator models in simulators cause
...
the accuracy with which real hardware is reflected in simulation. Furthermore, reward
...
\begin{itemize}
    \item Actuator modeling that reflects the coupled relationship between joints and actuators.
    \item Integration of a motor-side CoT estimate into training.
\end{itemize}
By incorporating these improvements, we aim to retain the benefits of complex hardware architectures
while supporting future sim-to-real transfer.
...
\thinhline
$\omega_{\max}$ [rad/s]            & 12.0 & 10.5 & 14.1$^{*}$ & 22.2$^{*}$ & 11.5$^{*}$ \\
\thinhline
\rowcolor{gray!7}
...
\thinhline
\rowcolor{gray!7}
$C_{p}$ [W/(Nm)$^{2}$]             & 3.54 & 33.44 & \multicolumn{2}{c|}{124.57} & 124.57 \\
\thinhline
...
Fig.~\ref{fig:ca_envelopes} shows representative operating points of the ankle motors during the agility
test. Because the CA structure breaks the one-to-one mapping between motor
...
torque--speed envelope using the Isaac Lab DCMotorActuator [25]; finally, the clipped motor torques are
...
\begin{equation}
    ...
    \label{eq:friction}
\end{equation}
where $v_{m} = 0.1$~rad/s is the velocity scale that smooths the sign transition of the Coulomb term.

To isolate the contribution of the active toe to energy efficiency, we estimate and minimize the CoT
directly, decomposing the motor-side energy consumption during locomotion into Joule heating and the
(signed) mechanical power:
\begin{equation}
...
\begin{itemize}
    ...
    \item $P_{\text{Joule}} = \sum_{i} C_{p,i}\,\tau_{i}^{2}$: resistive heating.
    \item $P_{\text{mech}} = \sum_{i} \max(p_{i},0)/\eta_{out} + \eta_{regen}\min(p_{i},0)$:
          battery-side mechanical power, $p_{i} = \tau_{i}\omega_{i}$,
          $\eta_{out} = 0.9$, $\eta_{regen} = 0.8$ (Table~I).
\end{itemize}


\subsection{Reinforcement Learning Setup}
To ensure a fair training procedure between the configurations with and without the active toe,
both models are given the same observation space. The policy, network, and reward-function
hyperparameters are summarized in Table~\ref{tab:rl_setup}. In the toe-ablation variant the toe
revolute joint is replaced by a fixed joint at the neutral ($0^{\circ}$) pose in the identical
joint frame; the toe link, its mass ($0.22$~kg per foot), inertia, and all collision geometry are
retained, so the total mass ($32.0$~kg) and the foot--ground contact geometry are unchanged. Only
the actuated DOF is removed, which reduces the per-leg CA Jacobian of Eq.~(1) from $7\times7$ to
$6\times6$.
...
in the same form as the recent thermal-aware reward proposed for quadruped locomotion [26]. As shown in
Table~\ref{tab:rl_setup}, $h_{i}$ denotes the safety margin of the $i$-th motor, defined as
$h_{i} = h^{\text{th}}_{i} - \mathrm{EMA}(\tau_{i}/\tau_{\text{cont},i})^{2}$, where $\tau_{\text{cont},i}$
is the motor continuous torque ($\tau_{\max}$ in Table~I) and $\tau_{\text{EMA}} = 3$\,s. The thresholds
are set heuristically to $h^{\text{th}}_{i} = 0.5$ for all motors.
...
\begin{table}[t]
    ...
    \hline
    Optimizer / framework  & \multicolumn{2}{l|}{PPO [24] / Isaac Lab [25]} \\
    ...
    Power consumption    & $P_{\text{Joule}} + P_{\text{mech}}$ (Eq.~(5))                                                 & $-10^{-4}$ \\
    ...
    \end{tabular}
    \begin{flushleft}
    {\scriptsize $[x]^{+} = \max(x,0)$; $\sigma_{\text{arr}} = 0.1$~m; $d^{\star}$: smallest goal
    distance reached so far in the episode. The arrival bonus is paid once, on the entering step.}
    \end{flushleft}
    \end{table}
...
straight-line walking trials over a 20~m path at 1.33~m/s for each configuration. Each configuration
is a single policy trained with one seed, and the ten trials are repeated rollouts of that
policy; all tabulated values are means over the ten trials. In Fig.~\ref{fig:validation}, the toe-ablation
...
Table~\ref{tab:quant_results} summarizes the whole-body metrics: relative to toe-ablation, toe-equipped reduces total power by 16.9\% (313.0~W to 260.2~W), CoT by 17.5\% (0.776 to 0.640), and heel-strike
GRF (resultant magnitude) by 5.0\% (1021.1~N to 970.0~N). Fig.~\ref{fig:cycle_average}
...
\begin{table}[t]
    ...
    Heel-strike GRF (avg.\ peak) [N]  & 970.0 & 1021.1 & $-$5.0 \\
    \bottomrule
    \end{tabular}
    \begin{flushleft}
    {\scriptsize $\Delta$: relative change of toe-equipped vs.\ toe-ablation. Mechanical loss:
    the cycle-averaged $P_{mech}$ of Eq.~(5).}
    \end{flushleft}
\end{table}
...
\begin{figure}[t]
    ...
    shaded bands are $\pm1$ standard deviation across gait cycles.}
    \label{fig:cycle_average}
\end{figure}

\subsection{Agility Test}

To assess how active toes affect directional-change performance, we propose a Robot T-Test inspired by the athletic
Agility T-Test [27]. Unlike the human test, the robot may rotate its torso freely and is trained with
...
Fig.~\ref{fig:ttest}(b) and (c) show the
trajectories from the T-test trials; compared with the toe-equipped case, the toe-ablation trajectories
depart from the reference path more often.
...
\begin{figure}[t]
    ...
    \end{subfigure}
    \caption{T-test comparison. (a) Course layout with waypoints
    A$\rightarrow$B$\rightarrow$C$\rightarrow$D$\rightarrow$B$\rightarrow$A; (b)/(c) overhead
    trajectories of ten trials, colored by instantaneous path deviation (green: on path, red:
    $\geq 0.5$~m).}
    \label{fig:ttest}
\end{figure}
...
This paper presented a 14-DOF bipedal robot with active toes and CA, and compared toe-equipped and
toe-ablation configurations trained under identical rewards and transmission-aware actuator models.
At 1.33\,m/s walking, the toe-equipped model reduced total power by 16.9\%, CoT by 17.5\%, and
heel-strike GRF by 5.0\%. On the Robot T-Test its average speed was 4.4\% lower, yet average and maximum
path deviation were 25.0\% and 34.0\% smaller at essentially unchanged completion time.

Although we aimed to build a realistic simulation environment, the link inertial parameters, backlash,
belt and frame stiffness, and the foot--ground contact model have not been validated against
hardware. Refining the hardware from these simulation insights, verifying sim-to-real transfer on the
physical robot, and broader comparisons across platforms, gaits, and terrains remain future work.

\section*{ACKNOWLEDGMENT}
This research was supported by the Future Mobility Project of WIRobotics Inc.
...
Mechatronics (ICROM)}, pp. 7--12, 2015.
\bibitem{ref17}
...
\bibitem{ref27}
K. Pauole, K. Madole, J. Garhammer, M. Lacourse, and R. Rozenek, ``Reliability and validity of the T-test as a measure of agility, leg power, and leg speed in college-aged men and women,'' \textit{J. Strength
Cond. Res.}, vol. 14, no. 4, pp. 443--450, Nov. 2000.
