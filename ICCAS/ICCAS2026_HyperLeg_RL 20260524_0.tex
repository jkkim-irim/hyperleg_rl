...
\begin{itemize}
    ...
    \item $P_{\text{mech}}$: battery-side mechanical power; each motor's
          $p_{i} = \tau_{i}\omega_{i}$ is charged as $p_{i}/\eta_{out}$ when motoring
          ($p_{i} \geq 0$) and credited as $\eta_{regen}\,p_{i}$ when regenerating,
          with $\eta_{out} = 0.9$ and $\eta_{regen} = 0.8$ (Table~I).
\end{itemize}
...
toe-ablation. Mechanical loss is the cycle-averaged battery-side mechanical term of Eq.~(5), i.e.,
the net sum of every motor's charged and regenerated power.} summarizes the whole-body metrics: relative to toe-ablation, toe-equipped reduces total power by 16.9\% (313.0~W to 260.2~W), CoT by 17.5\% (0.776 to 0.640), and heel-strike
