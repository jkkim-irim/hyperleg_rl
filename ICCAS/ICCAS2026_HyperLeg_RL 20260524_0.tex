...
are set heuristically to $h^{\text{th}}_{i} = 0.5$ for all motors, to prevent the policy from
...
is a single policy trained with one random seed, and the ten trials are repeated rollouts of that
...
toe-ablation. Mechanical loss is the cycle-averaged $P_{mech}$ of Eq.~(5).} summarizes the whole-body metrics: relative to toe-ablation, toe-equipped reduces total power by 16.9\% (313.0~W to 260.2~W), CoT by 17.5\% (0.776 to 0.640), and heel-strike
