...
\begin{equation}
    ...
    P_{mech} = \sum_{i}
    \begin{cases}
        p_{i}/\eta_{out}, & p_{i} \geq 0\\
        \eta_{regen}\, p_{i}, & p_{i} < 0
    \end{cases}
    \label{eq:cot}
\end{equation}
where $p_{i} = \tau_{i}\omega_{i}$ is the mechanical power of motor $i$, and:
\begin{itemize}
    \item $m$, $g$, $v$: robot mass, gravity, and velocity.
    \item $P_{\text{Joule}} = \sum_{i} C_{p,i}\,\tau_{i}^{2}$: resistive heating.
    \item $\eta_{out} = 0.9$, $\eta_{regen} = 0.8$: driver efficiencies (Table~I), so
          regenerated power is only partially credited back to the battery.
\end{itemize}
