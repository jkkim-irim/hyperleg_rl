    ...
    if friction_distribution_params is not None:
        for actuator in asset.actuators.values():
            ...
            if sum(actuator_joint_ids) > 0:
                friction = actuator.friction_coulomb.to(asset.device).clone()
                ...
                actuator.friction_coulomb[env_ids[:, None], actuator_joint_ids] = friction

                friction = actuator.friction_viscous.to(asset.device).clone()
                friction = _randomize_prop_by_op(
                    friction, friction_distribution_params, env_ids, torch.arange(friction.shape[1]), operation=operation, distribution=distribution
                )[env_ids][:, actuator_joint_ids]
                actuator.friction_viscous[env_ids[:, None], actuator_joint_ids] = friction